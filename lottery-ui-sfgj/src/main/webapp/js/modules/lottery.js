		    ...
		    $scope.queryExpertPushNum = function(lotteryType){
			 ...
			 $scope.getNowFormatDate = function(){
				...
				$scope.selRowFun = function(){
					...
					for(var i = 0; i < selRow.bitNum.length; i++){
						var selRow_ = {};
						selRow_.id = selRow.bitNum[i].playTypeId * 1;
						selRow_.label = selRow.bitNum[i].bettingNumKey;
						
						selRowArray.push(selRow_);
					}
					$scope.selRow = selRowArray;
					
					$scope.selRowFull = selRow;
				};
				
				$scope.produceSchedule = function(){
					//var playTypeId_ = $('#playTypeId_').val();
					var playTypeId_ = $scope.playTypeId_;
					...
					$('tr.ng-scope[data-type="10" ]').each(function(){
						...
						currBetAmount = currBetAmount * currBetTimes_;
						/*if(isIncrease_ == 0){
							amount_ = betAmount_ ;
						}else{
							amount_ = betAmount_ * currBetTimes_;
						}*/
						
						$('#zhAmount_'+dataIndex_).val(currBetAmount);
						
						if( isPrize_ == 1){
							currBetTimes_ = 1;
							currBetAmount = betAmount_ * 1;
						}else{
							currBetTimes_ = betTimes_ * 1;
						}
				...
				$scope.selPlayTypeFunc = function(playTypeId){
					//var playTypeId_ = $('#playTypeId_').val();
					for(var i = 0; i < $scope.selRowFull.bitNum.length; i++){
						var row_ = $scope.selRowFull.bitNum[i];
						if(row_.playTypeId == playTypeId.id && row_.bettingNumKey == playTypeId.label){
							$("#bettingNum_").val(row_.bettingNumVal);
							return row_.bettingNumVal;
						}
					}
				};
				...
				$scope.submitSchedule = function(lotteryType){
					...
					var playTypeId = $scope.playTypeId_;
					var bet = null;
					var bets = new Array();
					var times = 1;
					var pattern = 1;
					...
					if(typeof playTypeId == 'undefined' || playTypeId.length == 0 ){
						showToast("请选择号码!!");
						return ;
					}				
					...
					$('tr.ng-scope[data-type="10" ]').each(function(){
						...
						for(var i = 0; i < $scope.playTypeId_.length; i++){
							bet = {};
							
							var betNum_ = $scope.selPlayTypeFunc($scope.playTypeId_[i]);
							times = betAmount / pattern;
							bet.issueId = issueId;
							bet.playType = $scope.playTypeId_[i].id;
							bet.betNum = betNum_;
							bet.times = times;
							bet.pattern = pattern;
							bet.isZh = isZh;
