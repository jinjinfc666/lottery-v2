...
//		TeamPlReport selfProfit = querySelfProfit(startTime, endTime, user);
	...
	private TeamPlReport querySelfProfit(String startTime, String endTime, UserInfo user) {
		...
		sqlBuffer.append("select userInfo.user_name,userInfo.user_type, t.* from ")
		.append("(")
		.append("select ")
		.append("user_id,")
		.append("SUM(consumption) as consumption,")
	...
	private List<TeamPlReport> queryUserNextLevelProfit(String startTime, String endTime, UserInfo user) {
		...
		.append("from team_pl_report ")
		.append("where ( ")
		.append("(user_id in (select id from user_info where FIND_IN_SET(?,superior) = 1 and user_type = 7) ) ")
		.append(" or ")
	...
	public Map<String,Object> queryDailySettlement(String startTime, String endTime, UserInfo userInfo,Integer pageIndex,Integer pageSize) {
		...
		if(userInfo.getUserType().equals(Constants.UserType.GENERAL_AGENCY.getCode())){
			selfProfit = new TeamPlReport();
			selfProfit.setUserId(userInfo.getId());
		...
		if(userInfo.getUserType().equals(Constants.UserType.GENERAL_AGENCY.getCode())
				&& nextAgencyLevelProfit.size() == 1){
			nextAgencyLevelProfit.clear();
		}
		map.put("data", nextAgencyLevelProfit);
		map.put(Message.KEY_STATUS, Message.status.SUCCESS.getCode());
		return map;
	...
	private TeamPlReport querySelfSettlement(String startTime, String endTime, UserInfo user) {
	    ...
	    /*if(memberPlReport == null){
	    	memberPlReport = new TeamPlReport();
	    	memberPlReport.setUserId(user.getId());
	    	memberPlReport.setUserName(user.getUserName());
	    	memberPlReport.setUserType(user.getUserType());
	    }*/
...
//		select report.user_id,substring_index(group_concat(report.create_time order by report.create_time desc),",",1) as create_time from team_pl_report report,user_info userInfo where report.user_id = userInfo.id and find_in_set(1,userInfo.superior) = 1 group by user_id order by create_time desc
		...
		.append("report.user_id,substring_index(group_concat(report.create_time order by report.create_time desc),\",\",1) as create_time ")
		.append("from ")
	...
	private List<TeamPlReport> queryUserNextLevelSettlement(String startTime, String endTime, UserInfo user) {
		Integer id = user.getId();
		StringBuffer sqlBuffer = new StringBuffer();
		PageBean<Object[]> page = new PageBean<>();
		List<Object> params = new ArrayList<>();
...
//		select report.user_id,substring_index(group_concat(report.create_time order by report.create_time desc),",",1) as create_time from team_pl_report report,user_info userInfo where report.user_id = userInfo.id and find_in_set(1,userInfo.superior) = 1 group by user_id order by create_time desc
		...
		.append("team_pl_report report,user_info userInfo ")
		.append("where ")
		.append("report.user_id = userInfo.id and find_in_set(?,userInfo.superior) = 1 and report.create_time >=? and report.create_time <=? and report.user_type=7 ")
		.append("group by user_id order by create_time desc ");
	...
	public PageBean<TeamPlReport> queryDailySettlementByUser(String startTime, String endTime, String userName, String settlementFlag,
			Integer pageIndex, Integer pageSize) {
		StringBuffer buffer = new StringBuffer();
		Map<String,Object> map=new HashMap<String,Object>();
		
		buffer.append("from TeamPlReport where 1=1 ");
		...
		if(!StringUtils.isBlank(settlementFlag)) {
			buffer.append(" and settlementFlag=:settlementFlag ");
		...
		PageBean<TeamPlReport> page=new PageBean<>();
		page.setPageIndex(pageIndex);
		page.setPageSize(pageSize);
		
		PageBean<TeamPlReport> pageBean = queryByPagination(page, buffer.toString(),map);
		return pageBean;
	}
	@Override
