...
//@Service
//@Transactional
public class LuckyAirshipServiceImpl extends DefaultLottoTypeServiceImpl
{
	private Logger logger = Logger.getLogger(LuckyAirshipServiceImpl.class);

	private final String lotteryType = "xyft";
	...
	public List<Issue> makeAPlan() {
		...
		Date issueNumDate = null;
		
		today = DateUtil.addMinutes(today, 10);
		lastDay = DateUtil.addDay(today, -1);
		...
		for(int i = 0; i < maxAmount; i++) {
			...
			issue.setIssueNum(generateLottoNumber(issueNum, issueNumDate));
						...
						if(result.get("responseBody") != null) {
							...
							if(response.contains(issueNum.replace("-", ""))) {
								if(response.contains("preDrawCode")) {//网易
									...
									winningNum = parse168(response, format.format(new Date())+ issueNum.replace("-", ""), "10057");
								}
