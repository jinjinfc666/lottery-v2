...
//@Service
//@Transactional
public class WelfareLottery3DServiceImpl extends DefaultLottoTypeServiceImpl
{
	private Logger logger = Logger.getLogger(WelfareLottery3DServiceImpl.class);

	private final String lotteryType = "fc3d";
	...
	public List<Issue> makeAPlan() {		
		...
		GenSequence seq = seqServ.queryFC3DSeqVal();
		Long seqVal = seq.getSeqVal().longValue();
		long incresVal = 0;
		...
		*/
		...
		incresVal = DateUtil.intervalDays(seq.getCreateTime(), new Date()) - 1;
		incresVal *= maxAmount;
		seqVal += incresVal;
