...
//@Service
//@Transactional
public class OnePointFiveServiceImpl extends DefaultPrivateLottoTypeServiceImpl
{
	private Logger logger = Logger.getLogger(OnePointFiveServiceImpl.class);

	private final String lotteryType = "yfb";
	
	private String codeTypeName = Constants.SysCodeTypes.LOTTERY_CONFIG_YFB.getCode();
	...
	public void nonInterventional(String lottoType, String issueNum) {
		...
		List<String> winningNums = null;
		
		
		winningNums = cacheServ.get5DigitsOne2TenNumbers();
		
		int winningNumIndex = random.nextInt(winningNums.size());
		winningNum = winningNums.get(winningNumIndex);
