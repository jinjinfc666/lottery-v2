package com.jll.pay.hezPay;

import java.util.Date;

public class ScanPayParam
{
  private long id;
  
  private String userName;
  
  private String password1;
  
  private String password2;
  
  private String nickName;
  
  private float userMoney ;
  
  private float iceMoney ;
  
  private float allMoney ;
  
  private float creditMoney ;
  
  private int points ;
  
  private int addLimit ;
  
  private String qq ;
  
  private String email ;
  
  private String regFrom ;
  
  private String isdaili ;
  
  private String isOnline ;
  
  private String lastLogIp ;
  
  private Date lastLogTime ;
  
  private Date lastLogOut ;
  
  private int logins ;
  
  private Date addTime ;
  
  private float fenhong ;
  
  private String isLock ;
  
  private String bankName ;
  
  private String bankArea ;
  
  private String bankAccount ;
  
  private String bankNum ;
  
  private String isBankLock ;
  
  private String is17 ;
  
  private String is18 ;
  
  private String is19 ;
  
  private String iszy ;
  
  private String webSite ;
  
  private String sjfy ;
  
  private String ip ;
  
  private String jdnote ;
  
  private String djnote ;
  
  private String ifopenlhc ;
  
  private String orderLock ;
  
  private String ifUpdatepwd ;
  
  private String ifUpdatePwdDisplay ;
  
  private String ifEncode ;
  
  private float activityMoney ;
  
  private String bankBrand ;

public long getId() {
	return id;
}

public void setId(long id) {
	this.id = id;
}

public String getUserName() {
	return userName;
}

public void setUserName(String userName) {
	this.userName = userName;
}

public String getPassword1() {
	return password1;
}

public void setPassword1(String password1) {
	this.password1 = password1;
}

public String getPassword2() {
	return password2;
}

public void setPassword2(String password2) {
	this.password2 = password2;
}

public String getNickName() {
	return nickName;
}

public void setNickName(String nickName) {
	this.nickName = nickName;
}

public float getUserMoney() {
	return userMoney;
}

public void setUserMoney(float userMoney) {
	this.userMoney = userMoney;
}

public float getIceMoney() {
	return iceMoney;
}

public void setIceMoney(float iceMoney) {
	this.iceMoney = iceMoney;
}

public float getAllMoney() {
	return allMoney;
}

public void setAllMoney(float allMoney) {
	this.allMoney = allMoney;
}

public float getCreditMoney() {
	return creditMoney;
}

public void setCreditMoney(float creditMoney) {
	this.creditMoney = creditMoney;
}

public int getPoints() {
	return points;
}

public void setPoints(int points) {
	this.points = points;
}

public int getAddLimit() {
	return addLimit;
}

public void setAddLimit(int addLimit) {
	this.addLimit = addLimit;
}

public String getQq() {
	return qq;
}

public void setQq(String qq) {
	this.qq = qq;
}

public String getEmail() {
	return email;
}

public void setEmail(String email) {
	this.email = email;
}

public String getRegFrom() {
	return regFrom;
}

public void setRegFrom(String regFrom) {
	this.regFrom = regFrom;
}

public String getIsdaili() {
	return isdaili;
}

public void setIsdaili(String isdaili) {
	this.isdaili = isdaili;
}

public String getIsOnline() {
	return isOnline;
}

public void setIsOnline(String isOnline) {
	this.isOnline = isOnline;
}

public String getLastLogIp() {
	return lastLogIp;
}

public void setLastLogIp(String lastLogIp) {
	this.lastLogIp = lastLogIp;
}

public Date getLastLogTime() {
	return lastLogTime;
}

public void setLastLogTime(Date lastLogTime) {
	this.lastLogTime = lastLogTime;
}

public Date getLastLogOut() {
	return lastLogOut;
}

public void setLastLogOut(Date lastLogOut) {
	this.lastLogOut = lastLogOut;
}

public int getLogins() {
	return logins;
}

public void setLogins(int logins) {
	this.logins = logins;
}

public Date getAddTime() {
	return addTime;
}

public void setAddTime(Date addTime) {
	this.addTime = addTime;
}

public float getFenhong() {
	return fenhong;
}

public void setFenhong(float fenhong) {
	this.fenhong = fenhong;
}

public String getIsLock() {
	return isLock;
}

public void setIsLock(String isLock) {
	this.isLock = isLock;
}

public String getBankName() {
	return bankName;
}

public void setBankName(String bankName) {
	this.bankName = bankName;
}

public String getBankArea() {
	return bankArea;
}

public void setBankArea(String bankArea) {
	this.bankArea = bankArea;
}

public String getBankAccount() {
	return bankAccount;
}

public void setBankAccount(String bankAccount) {
	this.bankAccount = bankAccount;
}

public String getBankNum() {
	return bankNum;
}

public void setBankNum(String bankNum) {
	this.bankNum = bankNum;
}

public String getIsBankLock() {
	return isBankLock;
}

public void setIsBankLock(String isBankLock) {
	this.isBankLock = isBankLock;
}

public String getIs17() {
	return is17;
}

public void setIs17(String is17) {
	this.is17 = is17;
}

public String getIs18() {
	return is18;
}

public void setIs18(String is18) {
	this.is18 = is18;
}

public String getIs19() {
	return is19;
}

public void setIs19(String is19) {
	this.is19 = is19;
}

public String getIszy() {
	return iszy;
}

public void setIszy(String iszy) {
	this.iszy = iszy;
}

public String getWebSite() {
	return webSite;
}

public void setWebSite(String webSite) {
	this.webSite = webSite;
}

public String getSjfy() {
	return sjfy;
}

public void setSjfy(String sjfy) {
	this.sjfy = sjfy;
}

public String getIp() {
	return ip;
}

public void setIp(String ip) {
	this.ip = ip;
}

public String getJdnote() {
	return jdnote;
}

public void setJdnote(String jdnote) {
	this.jdnote = jdnote;
}

public String getDjnote() {
	return djnote;
}

public void setDjnote(String djnote) {
	this.djnote = djnote;
}

public String getIfopenlhc() {
	return ifopenlhc;
}

public void setIfopenlhc(String ifopenlhc) {
	this.ifopenlhc = ifopenlhc;
}

public String getOrderLock() {
	return orderLock;
}

public void setOrderLock(String orderLock) {
	this.orderLock = orderLock;
}

public String getIfUpdatepwd() {
	return ifUpdatepwd;
}

public void setIfUpdatepwd(String ifUpdatepwd) {
	this.ifUpdatepwd = ifUpdatepwd;
}

public String getIfUpdatePwdDisplay() {
	return ifUpdatePwdDisplay;
}

public void setIfUpdatePwdDisplay(String ifUpdatePwdDisplay) {
	this.ifUpdatePwdDisplay = ifUpdatePwdDisplay;
}

public String getIfEncode() {
	return ifEncode;
}

public void setIfEncode(String ifEncode) {
	this.ifEncode = ifEncode;
}

public float getActivityMoney() {
	return activityMoney;
}

public void setActivityMoney(float activityMoney) {
	this.activityMoney = activityMoney;
}

public String getBankBrand() {
	return bankBrand;
}

public void setBankBrand(String bankBrand) {
	this.bankBrand = bankBrand;
}
  

}
